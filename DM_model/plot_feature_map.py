...
def get_conv1_feature_maps(model, input_image):
    ...
    with torch.no_grad():  # No need to calculate gradients for visualization
        _ = model(input_image.unsqueeze(0))  # Pass the input image through the model
...
model = loader("20240912_172513", "69")

image = 'A6'

# Load an input image and preprocess it
transform = transforms.Compose([transforms.Resize((32, 32)), transforms.ToTensor()])
input_image = Image.open(f"./DM_model/DM datasets/DM/{image}.png")  # Replace with your image path
